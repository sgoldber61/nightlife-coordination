...
exports.addBar = function(req, res, next) {
  ...
  Bar.findOne({id, users: user_id}, function(err, bar) {
    ...
    if (bar) {
      return res.status(422).send({error: "User is already at the bar and cannot join."});
    }
...
exports.removeBar = function(req, res, next) {
  ...
  Bar.findOneAndUpdate({id, users: user_id}, {$pull: {users: user_id}, $inc: {totalUsers: -1}}, {new: true}, function(err, bar) {
    ...
    if (!bar) {
      return res.status(422).send({error: "User is not at the bar and cannot be removed."});
    }
    ...
    if (bar.users.length == 0) {
      bar.remove(function(err, bar) {
        ...
        return res.json({message: "User was removed from the bar, and empty bar has been deleted."});
      });
    }
    else {
...
exports.barsUserData = function(req, res, next) {
  ...
  barDataQuery(yelpIds, user_id).exec(function(err, userData) {
    ...
    return res.json({message: "Bars user data returned.", userData});
  });
};
